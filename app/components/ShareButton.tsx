...
export default function ShareButton({ meetupId, title, date }: ShareButtonProps) {
  ...
  const shareText = `Join us for ${title} on ${date}!`;
  ...
  const shareTargets = [
    ...
    },
    {
      ...
      action: () => window.open(`https://x.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}`, "_blank"),
      label: "X / Twitter",
    },
    {
      name: "WhatsApp",
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-700 shadow-sm transition-all hover:border-orange-300 hover:bg-orange-50 hover:text-orange-700"
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-xl border border-zinc-200 bg-white p-2 shadow-lg">
          {shareTargets.map((target) => (
            <button
              key={target.name}
              onClick={() => {
                target.action();
                if (target.name !== "Copy Link") setOpen(false);
              }}
              className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 hover:text-zinc-900"
            >
